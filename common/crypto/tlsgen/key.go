...
func newSM2PrivKey() (*sm2.PrivateKey, []byte, error) {
	...
	privBytes, err := GMX509.MarshalECPrivateKey(privateKey)
	if err != nil {
		return nil, nil, err
	}
	return privateKey, privBytes, nil
...
func newSM2CertTemplate() (GMX509.Certificate, error) {
	sn, err := rand.Int(rand.Reader, new(big.Int).Lsh(big.NewInt(1), 128))
	if err != nil {
...
func newSM2CertKeyPair(isCA bool, isServer bool, host string, certSigner crypto.Signer, parent interface{}) (*CertKeyPair, error) {
	...
	rawBytes, err := GMX509.CreateCertificate(rand.Reader, &template, parent.(*GMX509.Certificate), &privateKey.PublicKey, certSigner)
	if err != nil {
		return nil, err
	}
	pubKey := encodePEM("CERTIFICATE", rawBytes)
