...
type IGRPCClient interface {
	NewConnection(address string, tlsOptions ...interface{}) (*grpc.ClientConn, error)
	Certificate() interface{}
	CertificateHash() []byte
	MutualTLSRequired() bool
	ServerNameOverride(name string) interface{}
	CertPoolOverride(pool interface{}) interface{}
}
...
func NewGRPCClient(config ClientConfig) (IGRPCClient, error) {
	if IsGM() {
		client := &GMGRPCClient{}
...
func (client *GMGRPCClient) parseSecureOptions(opts SecureOptions) error {
	// if TLS is not enabled, return
	if !opts.UseTLS {
		return nil
	}
	...
	if len(opts.ServerRootCAs) > 0 {
		...
		for _, certBytes := range opts.ServerRootCAs {
			err := AddGMPemToCertPool(certBytes, client.tlsConfig.RootCAs)
			if err != nil {
				commLogger.Debugf("error adding root certificate: %v", err)
				return errors.WithMessage(err, "error adding root certificate")
			}
...
// Certificate returns the tls.Certificate used to make TLS connections
// when client certificates are required by the server
func (client *GMGRPCClient) Certificate() interface{} {
...
func (client *GMGRPCClient) MutualTLSRequired() bool {
	return client.tlsConfig != nil &&
		len(client.tlsConfig.Certificates) > 0
}
...
type TLSGMOption func(tlsConfig *gmtls.Config)


func (client *GRPCClient) ServerNameOverride(name string) interface{} {
	return func(tlsConfig *tls.Config) {
		tlsConfig.ServerName = name
	}
}

func (client *GMGRPCClient) ServerNameOverride(name string) interface{} {
	return func(tlsConfig *gmtls.Config) {
		tlsConfig.ServerName = name
	}
}

func (client *GRPCClient) CertPoolOverride(pool interface{}) interface{} {
	return func(tlsConfig *tls.Config) {
		tlsConfig.RootCAs = pool.(*x509.CertPool)
	}
}

func (client *GMGRPCClient) CertPoolOverride(pool interface{}) interface{} {
	return func(tlsConfig *gmtls.Config) {
		tlsConfig.RootCAs = pool.(*sm2.CertPool)
	}
}
...
func (client *GRPCClient) NewConnection(address string, tlsOptions ...interface{}) (*grpc.ClientConn, error) {

	var dialOpts []grpc.DialOption
	dialOpts = append(dialOpts, client.dialOpts...)

	var tlsOptionArray []TLSOption
	if tlsOptions != nil && len(tlsOptions) > 0 {
		for _,options := range tlsOptions {
			function := options.(func(tlsConfig *tls.Config))
			tlsOptionArray = append(tlsOptionArray,function)
		}
	}
...
// overrides the server name used to verify the hostname on the
// certificate returned by a server when using TLS
func (client *GMGRPCClient) NewConnection(address string, tlsOptions ...interface{}) (*grpc.ClientConn, error) {

	var dialOpts []grpc.DialOption
	dialOpts = append(dialOpts, client.dialOpts...)

	var tlsOptionArray []TLSGMOption
	if tlsOptions != nil && len(tlsOptions) > 0 {
		for _,options := range tlsOptions {
			function := options.(func(tlsConfig *gmtls.Config))
			tlsOptionArray = append(tlsOptionArray,function)
		}
	}
...
func (client *GMGRPCClient) CertificateHash() []byte {
	cert := client.Certificate()
	switch cert.(type) {
	case gmtls.Certificate:
		if len(cert.(gmtls.Certificate).Certificate) > 0 {
			return util.ComputeSHA256(cert.(gmtls.Certificate).Certificate[0])
		}else {
			return nil
		}
	default:
...
func (client *GRPCClient) CertificateHash() []byte {
	cert := client.Certificate()
	switch cert.(type) {
	case tls.Certificate:
		if len(cert.(tls.Certificate).Certificate) > 0 {
			return util.ComputeSHA256(cert.(tls.Certificate).Certificate[0])
		}else {
			return nil
		}
	default:
