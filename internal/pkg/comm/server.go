...
type IGRPCServer interface {
	...
	appendClientRootCA(clientRoot []byte) error
...
// listen address
func NewGRPCServer(address string, serverConfig ServerConfig) (IGRPCServer, error) {
	if address == "" {
...
func NewGRPCServerFromListener(listener net.Listener, serverConfig ServerConfig) (IGRPCServer, error) {
	if IsGM() {
		return NewGRPCGMServerFromListener(listener, serverConfig)
	}
...
// NewGRPCServerFromListener creates a new implementation of a GRPCServer given
// an existing net.Listener instance using default keepalive
func NewGRPCGMServerFromListener(listener net.Listener, serverConfig ServerConfig) (IGRPCServer, error) {
	grpcServer := &GMGRPCServer{
		address:  listener.Addr().String(),
		listener: listener,
		lock:     &sync.Mutex{},
	}
	...
	if secureConfig.UseTLS {
		...
		if secureConfig.Key != nil && secureConfig.Certificate != nil {
			...
			//TODO
			grpcServer.tls = NewTLSConfigGM(&gmtls.Config{
				//VerifyPeerCertificate:  secureConfig.VerifyCertificate,
				GetCertificate:         getCert,
				SessionTicketsDisabled: true,
				CipherSuites:           secureConfig.CipherSuites,
			...
			grpcServer.tls.config.ClientAuth = gmtls.RequestClientCert
			//check if client authentication is required
			if secureConfig.RequireClientCert {
				//require TLS client auth
				grpcServer.tls.config.ClientAuth = gmtls.RequireAndVerifyClientCert
				//if we have client root CAs, create a certPool
				if len(secureConfig.ClientRootCAs) > 0 {
...
func (gServer *GMGRPCServer) Address() string {
	return gServer.address
}
...
func (gServer *GMGRPCServer) Server() *grpc.Server {
	return gServer.server
}
...
func (gServer *GMGRPCServer) ServerCertificate() interface{} {
	return gServer.serverCertificate.Load().(gmtls.Certificate)
}
...
func (gServer *GMGRPCServer) TLSEnabled() bool {
	return gServer.tls != nil
}
...
func (gServer *GMGRPCServer) MutualTLSRequired() bool {
	return gServer.TLSEnabled() &&
		gServer.tls.Config().ClientAuth == gmtls.RequireAndVerifyClientCert
}
...
func (gServer *GMGRPCServer) Start() error {
	// if health check is enabled, set the health status for all registered services
	if gServer.healthServer != nil {
		for name := range gServer.server.GetServiceInfo() {
			gServer.healthServer.SetServingStatus(
...
func (gServer *GMGRPCServer) Stop() {
	gServer.server.Stop()
}
...
func (gServer *GMGRPCServer) appendClientRootCA(clientRoot []byte) error {
...
func (gServer *GMGRPCServer) SetClientRootCAs(clientRoots [][]byte) error {
	gServer.lock.Lock()
	defer gServer.lock.Unlock()
