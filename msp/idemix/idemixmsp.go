...
import (
	...
	msp1 "github.com/hyperledger/fabric/msp"
	"time"
...
func (msp *idemixmsp) Setup(conf1 *m.MSPConfig) error {
	...
	if conf1.Type != int32(msp1.IDEMIX) {
...
func (msp *idemixmsp) GetVersion() msp1.MSPVersion {
	return msp.version
}

func (msp *idemixmsp) GetType() msp1.ProviderType {
	return msp1.IDEMIX
}
...
func (msp *idemixmsp) GetSigningIdentity(identifier *msp1.IdentityIdentifier) (msp1.SigningIdentity, error) {
	return nil, errors.Errorf("GetSigningIdentity not implemented")
}

func (msp *idemixmsp) GetDefaultSigningIdentity() (msp1.SigningIdentity, error) {
...
func (msp *idemixmsp) DeserializeIdentity(serializedID []byte) (msp1.Identity, error) {
...
func (msp *idemixmsp) deserializeIdentityInternal(serializedID []byte) (msp1.Identity, error) {
...
func (msp *idemixmsp) Validate(id msp1.Identity) error {
...
func (msp *idemixmsp) SatisfiesPrincipal(id msp1.Identity, principal *m.MSPPrincipal) error {
	err := msp.Validate(id)
	if err != nil {
...
func (msp *idemixmsp) satisfiesPrincipalValidated(id msp1.Identity, principal *m.MSPPrincipal) error {
...
func (id *idemixidentity) GetIdentifier() *msp1.IdentityIdentifier {
	return id.id
}
...
func (id *idemixidentity) GetOrganizationalUnits() []*msp1.OUIdentifier {
	// we use the (serialized) public key of this MSP as the CertifiersIdentifier
	certifiersIdentifier, err := id.msp.ipk.Bytes()
	if err != nil {
		//sw2.mspIdentityLogger.Errorf("Failed to marshal ipk in GetOrganizationalUnits: %s", err)
		return nil
	}

	return []*msp1.OUIdentifier{{certifiersIdentifier, id.OU.OrganizationalUnitIdentifier}}
}
...
func (id *idemixidentity) Verify(msg []byte, sig []byte) error {
	//if sw2.mspIdentityLogger.IsEnabledFor(zapcore.DebugLevel) {
	//	sw2.mspIdentityLogger.Debugf("Verify Idemix sig: msg = %s", hex.Dump(msg))
	//	sw2.mspIdentityLogger.Debugf("Verify Idemix sig: sig = %s", hex.Dump(sig))
	//}
...
func (id *idemixSigningIdentity) GetPublicVersion() msp1.Identity {
