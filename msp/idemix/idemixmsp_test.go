...
func setup(configPath string, ID string) (msp2.MSP, error) {
	return setupWithVersion(configPath, ID, msp2.MSPv1_3)
}

func setupWithVersion(configPath string, ID string, version msp2.MSPVersion) (msp2.MSP, error) {
	msp, err := NewIdemixMsp(version)
	if err != nil {
		return nil, err
	}

	conf, err := msp2.GetIdemixMspConfig(configPath, ID)
	if err != nil {
...
func getDefaultSigner(msp msp2.MSP) (msp2.SigningIdentity, error) {
...
func TestSetupBad(t *testing.T) {
	...
	msp1, err := NewIdemixMsp(msp2.MSPv1_3)
	assert.NoError(t, err)
	...
	conf = &msp.MSPConfig{Type: int32(msp2.IDEMIX), Config: []byte("barf")}
	err = msp1.Setup(conf)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "failed unmarshalling idemix msp config")

	conf, err = msp2.GetIdemixMspConfig("testdata/idemix/MSP1OU1", "IdemixMSP1")
	assert.NoError(t, err)
