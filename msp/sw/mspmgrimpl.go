...
type mspManagerImpl struct {
	...
	mspsMap map[string]msp2.MSP

	// map that maps MSPs by their provider types
	mspsByProviders map[msp2.ProviderType][]msp2.MSP
...
func (mgr *mspManagerImpl) Setup(msps []msp2.MSP) error {
	...
	mgr.mspsMap = make(map[string]msp2.MSP)

	// create the map that sorts MSPs by their provider types
	mgr.mspsByProviders = make(map[msp2.ProviderType][]msp2.MSP)
...
func (mgr *mspManagerImpl) GetMSPs() (map[string]msp2.MSP, error) {
	return mgr.mspsMap, nil
}

// DeserializeIdentity returns an identity given its serialized version supplied as argument
func (mgr *mspManagerImpl) DeserializeIdentity(serializedID []byte) (msp2.Identity, error) {
