...
func (msp *bccspmsp) setupCrypto(conf *m.FabricMSPConfig) error {
	...
	if msp.cryptoConfig == nil {
		...
		mspLogger.Debugf("SW CryptoConfig was nil. Move to defaults.")
	}
...
func (msp *bccspmsp) setupCAs(conf *m.FabricMSPConfig) error {
	...
	msp.rootCerts = make([]msp2.Identity, len(conf.RootCerts))
	...
	msp.intermediateCerts = make([]msp2.Identity, len(conf.IntermediateCerts))
...
func (msp *bccspmsp) setupAdminsPreV142(conf *m.FabricMSPConfig) error {
	...
	msp.admins = make([]msp2.Identity, len(conf.Admins))
...
func (msp *bccspmsp) finalizeSetupCAs() error {
	...
	for _, id := range append(append([]msp2.Identity{}, msp.rootCerts...), msp.intermediateCerts...) {
	...
	for _, id := range append([]msp2.Identity{}, msp.intermediateCerts...) {
...
func (msp *bccspmsp) setupNodeOUs(config *m.FabricMSPConfig) error {
	if config.FabricNodeOus != nil {
		...
		// ClientOU
		msp.clientOU = &msp2.OUIdentifier{OrganizationalUnitIdentifier: config.FabricNodeOus.ClientOuIdentifier.OrganizationalUnitIdentifier}
		...
		// PeerOU
		msp.peerOU = &msp2.OUIdentifier{OrganizationalUnitIdentifier: config.FabricNodeOus.PeerOuIdentifier.OrganizationalUnitIdentifier}
...
func (msp *bccspmsp) setupNodeOUsV142(config *m.FabricMSPConfig) error {
	...
	if config.FabricNodeOus.AdminOuIdentifier != nil {
		msp.adminOU = &msp2.OUIdentifier{OrganizationalUnitIdentifier: config.FabricNodeOus.AdminOuIdentifier.OrganizationalUnitIdentifier}
	...
	if config.FabricNodeOus.OrdererOuIdentifier != nil {
		msp.ordererOU = &msp2.OUIdentifier{OrganizationalUnitIdentifier: config.FabricNodeOus.OrdererOuIdentifier.OrganizationalUnitIdentifier}
