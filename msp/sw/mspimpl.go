...
func (msp *bccspmsp) getIdentityFromConf(idBytes []byte) (msp.Identity, bccsp.Key, error) {
	// get a cert
...
func (msp *bccspmsp) getSigningIdentityFromConf(sidInfo *m.SigningIdentityInfo) (msp.SigningIdentity, error) {
	if sidInfo == nil {
...
func (msp *bccspmsp) GetVersion() msp.MSPVersion {
	return msp.version
}

// GetType returns the type for this MSP
func (msp1 *bccspmsp) GetType() msp.ProviderType {
	return msp.FABRIC
}
...
func (msp *bccspmsp) GetDefaultSigningIdentity() (msp.SigningIdentity, error) {
...
func (msp *bccspmsp) GetSigningIdentity(identifier *msp.IdentityIdentifier) (msp.SigningIdentity, error) {
	// TODO
...
// error otherwise
func (msp *bccspmsp) Validate(id msp.Identity) error {
...
func (msp *bccspmsp) hasOURole(id msp.Identity, mspRole m.MSPRole_MSPRoleType) error {
	// Check NodeOUs
...
func (msp1 *bccspmsp) hasOURoleInternal(id *identity, mspRole m.MSPRole_MSPRoleType) error {
	var nodeOU *msp.OUIdentifier
	switch mspRole {
	case m.MSPRole_CLIENT:
		nodeOU = msp1.clientOU
	case m.MSPRole_PEER:
		nodeOU = msp1.peerOU
	case m.MSPRole_ADMIN:
		nodeOU = msp1.adminOU
	case m.MSPRole_ORDERER:
		nodeOU = msp1.ordererOU
	default:
		return errors.New("Invalid MSPRoleType. It must be CLIENT, PEER, ADMIN or ORDERER")
	}

	if nodeOU == nil {
		return errors.Errorf("cannot test for classification, node ou for type [%s], not defined, msp: [%s]", mspRole, msp1.name)
	}
	...
	return errors.Errorf("The identity does not contain OU [%s], MSP: [%s]", mspRole, msp1.name)
}

// DeserializeIdentity returns an Identity given the byte-level
// representation of a SerializedIdentity struct
func (msp *bccspmsp) DeserializeIdentity(serializedID []byte) (msp.Identity, error) {
...
func (msp *bccspmsp) deserializeIdentityInternal(serializedIdentity []byte) (msp.Identity, error) {
...
func (msp *bccspmsp) SatisfiesPrincipal(id msp.Identity, principal *m.MSPPrincipal) error {
...
func (msp *bccspmsp) satisfiesPrincipalInternalPreV13(id msp.Identity, principal *m.MSPPrincipal) error {
	switch principal.PrincipalClassification {
...
func (msp *bccspmsp) satisfiesPrincipalInternalV13(id msp.Identity, principal *m.MSPPrincipal) error {
	switch principal.PrincipalClassification {
	case m.MSPPrincipal_COMBINED:
...
func (msp *bccspmsp) satisfiesPrincipalInternalV142(id msp.Identity, principal *m.MSPPrincipal) error {
	_, okay := id.(*identity)
	if !okay {
...
func (msp *bccspmsp) getCertificationChain(id msp.Identity) ([]*x509.Certificate, error) {
...
func (msp *bccspmsp) getCertificationChainIdentifier(id msp.Identity) ([]byte, error) {
