...
func (msp1 *bccspmsp) validateIdentityOUsV11(id *identity) error {
	// Run the same checks as per V1
	err := msp1.validateIdentityOUsV1(id)
	if err != nil {
		return err
	}
	...
	for _, OU := range id.GetOrganizationalUnits() {
		...
		case msp1.clientOU.OrganizationalUnitIdentifier:
			nodeOU = msp1.clientOU
		case msp1.peerOU.OrganizationalUnitIdentifier:
			nodeOU = msp1.peerOU
		default:
			continue
		}
		...
		if len(nodeOU.CertifiersIdentifier) != 0 && !bytes.Equal(nodeOU.CertifiersIdentifier, OU.CertifiersIdentifier) {
			return errors.Errorf("certifiersIdentifier does not match: %v, MSP: [%s]", OUIDs(id.GetOrganizationalUnits()), msp1.name)
		}
		counter++
		if counter > 1 {
			break
	...
	if counter == 0 {
		return errors.Errorf("the identity does not have an OU that resolves to client or peer. OUs: %s, MSP: [%s]", OUIDs(id.GetOrganizationalUnits()), msp1.name)
	}
	if counter > 1 {
		return errors.Errorf("the identity must be a client or a peer identity to be valid, not a combination of them. OUs: %s, MSP: [%s]", OUIDs(id.GetOrganizationalUnits()), msp1.name)
	}

	return nil
}

func (msp1 *bccspmsp) validateIdentityOUsV142(id *identity) error {
	// Run the same checks as per V1
	err := msp1.validateIdentityOUsV1(id)
	if err != nil {
		return err
	...
	counter := 0
	validOUs := make(map[string]*msp.OUIdentifier)
	if msp1.clientOU != nil {
		validOUs[msp1.clientOU.OrganizationalUnitIdentifier] = msp1.clientOU
	}
	if msp1.peerOU != nil {
		validOUs[msp1.peerOU.OrganizationalUnitIdentifier] = msp1.peerOU
	}
	if msp1.adminOU != nil {
		validOUs[msp1.adminOU.OrganizationalUnitIdentifier] = msp1.adminOU
	}
	if msp1.ordererOU != nil {
		validOUs[msp1.ordererOU.OrganizationalUnitIdentifier] = msp1.ordererOU
	}
	...
	for _, OU := range id.GetOrganizationalUnits() {
		...
		if len(nodeOU.CertifiersIdentifier) != 0 && !bytes.Equal(nodeOU.CertifiersIdentifier, OU.CertifiersIdentifier) {
			return errors.Errorf("certifiersIdentifier does not match: %s, MSP: [%s]", OUIDs(id.GetOrganizationalUnits()), msp1.name)
		}
		counter++
		if counter > 1 {
			break
	...
	// the identity should have exactly one OU role, return an error if the counter is not 1.
	if counter == 0 {
		return errors.Errorf("the identity does not have an OU that resolves to client, peer, orderer, or admin role. OUs: %s, MSP: [%s]", OUIDs(id.GetOrganizationalUnits()), msp1.name)
	}
	if counter > 1 {
		return errors.Errorf("the identity must have a client, a peer, an orderer, or an admin OU role to be valid, not a combination of them. OUs: %s, MSP: [%s]", OUIDs(id.GetOrganizationalUnits()), msp1.name)
	}
