...
func (gmmsp *gmmsp) validateIdentity(id *identity) error {
...
func (gmmsp *gmmsp) validateCAIdentity(id *identity) error {
	if !id.cert.IsCA {
		return errors.New("Only CA identities can be validated")
	}

	validationChain, err := gmmsp.getUniqueValidationChain(id.cert, gmmsp.getValidityOptsForCert(id.cert))
	if err != nil {
	...
	return gmmsp.validateIdentityAgainstChain(id, validationChain)
}

func (gmmsp *gmmsp) validateTLSCAIdentity(cert *sm2.Certificate, opts *sm2.VerifyOptions) error {
	if !cert.IsCA {
		return errors.New("Only CA identities can be validated")
	}

	validationChain, err := gmmsp.getUniqueValidationChain(cert, *opts)
	if err != nil {
		return errors.WithMessage(err, "could not obtain certification chain")
	}
	if len(validationChain) == 1 {
	...
	return gmmsp.validateCertAgainstChain(cert, validationChain)
}

func (gmmsp *gmmsp) validateIdentityAgainstChain(id *identity, validationChain []*sm2.Certificate) error {
	return gmmsp.validateCertAgainstChain(id.cert, validationChain)
}

func (gmmsp *gmmsp) validateCertAgainstChain(cert *sm2.Certificate, validationChain []*sm2.Certificate) error {
...
func (gmmsp *gmmsp) validateIdentityOUsV1(id *identity) error {
	// Check that the identity's OUs are compatible with those recognized by this MSP,
	// meaning that the intersection is not empty.
	if len(gmmsp.ouIdentifiers) > 0 {
		found := false

		for _, OU := range id.GetOrganizationalUnits() {
			certificationIDs, exists := gmmsp.ouIdentifiers[OU.OrganizationalUnitIdentifier]
		...
		if !found {
			...
			return errors.Errorf("none of the identity's organizational units %s are in MSP %s", sw.OUIDs(id.GetOrganizationalUnits()), gmmsp.name)
		}
	}
...
func (gmmsp *gmmsp) validateIdentityOUsV11(id *identity) error {
	// Run the same checks as per V1
	err := gmmsp.validateIdentityOUsV1(id)
	if err != nil {
		return err
	}
	...
	for _, OU := range id.GetOrganizationalUnits() {
		...
		var nodeOU *msp2.OUIdentifier
		switch OU.OrganizationalUnitIdentifier {
		case gmmsp.clientOU.OrganizationalUnitIdentifier:
			nodeOU = gmmsp.clientOU
		case gmmsp.peerOU.OrganizationalUnitIdentifier:
			nodeOU = gmmsp.peerOU
		default:
			continue
		}
		...
		if len(nodeOU.CertifiersIdentifier) != 0 && !bytes.Equal(nodeOU.CertifiersIdentifier, OU.CertifiersIdentifier) {
			return errors.Errorf("certifiersIdentifier does not match: %v, MSP: [%s]", sw.OUIDs(id.GetOrganizationalUnits()), gmmsp.name)
		}
		counter++
		if counter > 1 {
			break
	...
	if counter == 0 {
		return errors.Errorf("the identity does not have an OU that resolves to client or peer. OUs: %s, MSP: [%s]", sw.OUIDs(id.GetOrganizationalUnits()), gmmsp.name)
	}
	if counter > 1 {
		return errors.Errorf("the identity must be a client or a peer identity to be valid, not a combination of them. OUs: %s, MSP: [%s]", sw.OUIDs(id.GetOrganizationalUnits()), gmmsp.name)
	}

	return nil
}

func (gmmsp *gmmsp) validateIdentityOUsV142(id *identity) error {
	// Run the same checks as per V1
	err := gmmsp.validateIdentityOUsV1(id)
	if err != nil {
		return err
	...
	counter := 0
	validOUs := make(map[string]*msp2.OUIdentifier)
	if gmmsp.clientOU != nil {
		validOUs[gmmsp.clientOU.OrganizationalUnitIdentifier] = gmmsp.clientOU
	}
	if gmmsp.peerOU != nil {
		validOUs[gmmsp.peerOU.OrganizationalUnitIdentifier] = gmmsp.peerOU
	}
	if gmmsp.adminOU != nil {
		validOUs[gmmsp.adminOU.OrganizationalUnitIdentifier] = gmmsp.adminOU
	}
	if gmmsp.ordererOU != nil {
		validOUs[gmmsp.ordererOU.OrganizationalUnitIdentifier] = gmmsp.ordererOU
	}
	...
	for _, OU := range id.GetOrganizationalUnits() {
		...
		if len(nodeOU.CertifiersIdentifier) != 0 && !bytes.Equal(nodeOU.CertifiersIdentifier, OU.CertifiersIdentifier) {
			return errors.Errorf("certifiersIdentifier does not match: %s, MSP: [%s]", sw.OUIDs(id.GetOrganizationalUnits()), gmmsp.name)
		}
		counter++
		if counter > 1 {
			break
	...
	// the identity should have exactly one OU role, return an error if the counter is not 1.
	if counter == 0 {
		return errors.Errorf("the identity does not have an OU that resolves to client, peer, orderer, or admin role. OUs: %s, MSP: [%s]", sw.OUIDs(id.GetOrganizationalUnits()), gmmsp.name)
	}
	if counter > 1 {
		return errors.Errorf("the identity must have a client, a peer, an orderer, or an admin OU role to be valid, not a combination of them. OUs: %s, MSP: [%s]", sw.OUIDs(id.GetOrganizationalUnits()), gmmsp.name)
	}

	return nil
}

func (gmmsp *gmmsp) getValidityOptsForCert(cert *sm2.Certificate) sm2.VerifyOptions {
	...
	tempOpts.Roots = gmmsp.opts.Roots
	tempOpts.DNSName = gmmsp.opts.DNSName
	tempOpts.Intermediates = gmmsp.opts.Intermediates
	tempOpts.KeyUsages = gmmsp.opts.KeyUsages
